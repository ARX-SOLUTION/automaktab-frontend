...
import { GraduationCap, DollarSign, AlertTriangle, Users, TrendingUp } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <Tabs value={courseType || 'all'} onValueChange={(v) => setCourseType(v === 'all' ? undefined : v as CourseType)}>
      ...
      {/* Cards */}
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        <SummaryCard title="Jami talabalar" value={analytics.total_students} icon={<GraduationCap className="h-5 w-5" />} trend="+12% o'tgan oyga" />
        {isOwner() && (
          <SummaryCard title="Jami daromad" value={formatSum(analytics.total_revenue)} icon={<DollarSign className="h-5 w-5" />} />
        )}
        <SummaryCard title="Kutilayotgan to'lovlar" value={formatSum(analytics.pending_payments)} icon={<AlertTriangle className="h-5 w-5" />} />
        <SummaryCard title="Faol kurslar" value={`${analytics.active_tezkor} Tezkor / ${analytics.active_avto} Avto`} icon={<Users className="h-5 w-5" />} />
      </div>
